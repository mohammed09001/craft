...
import { createSelectActiveMoldBodies, createSelectSpruePresentationDefinitions, selectActiveMoldBodies, selectSpruePresentationDefinitions, SplitFaceControls, useSplitFaceStore } from "@/features/mold-generation/split-face";
...
// Module-level (not created inside the component) so it's a stable,
// persistently-memoized selector across renders -- the Manual draft's own
// instance, independent of the singleton's `selectActiveMoldBodies` above.
const selectManualActiveMoldBodies = createSelectActiveMoldBodies();
const selectManualSpruePresentationDefinitions = createSelectSpruePresentationDefinitions();

// A stable empty reference so blocking sprue presentation during a
// more-molds session never produces a new array identity every render.
...
export function Viewport({ onStatusChange }: ViewportProps) {
  ...
  const isManualDraftActive = useIsManualDraftActive();
  // The single authoritative read of whether the unified Constructed
  // Cutting Plan panel is open, and which of its three tabs is active --
  ...
  const isOneMoldTabActive = isSessionOpen && activeSessionTab === "oneMold";
  const isMoreMoldsTabActive = isSessionOpen && activeSessionTab === "moreMolds";
  const moreMoldsActiveDraft = useCuttingWorkflowStore((s) =>
    s.state.kind === "sessionOpen" && s.state.activeTab === "moreMolds"
      ? s.state.moreMoldsActiveDraft
      : null,
  );
  // Cut by Face reuses the singleton directly (its own tab), and Manual
  // reuses its own draft -- every other open-session state (One Mold, More
  // Molds' Automatic sub-strategy) must never fall through to mutating the
  // singleton's hidden, not-currently-active data.
  ...
  const manualRemoveSplitFaceAndRebuild = useManualDraftStore(
    (state) => state.removeSplitFaceAndRebuild,
  );
  const removeSplitFaceAndRebuild = isManualDraftActive
    ? manualRemoveSplitFaceAndRebuild
    : singletonInteractionBlocked
  ...
  // contextually only once a segmentation tab that needs printer-aware
  // analysis is active.
  const printerBuildVolume = usePrinterBuildVolume();
  // Automatic's segmentation planning needs printer dimensions exactly like
  // the One Mold tab does -- Manual never requires them to enter or operate
  // its own workflow, so the prompt must not appear for Manual.
  const needsPrinterDimensionsPrompt =
    (isOneMoldTabActive || (isMoreMoldsTabActive && moreMoldsActiveDraft === "automatic")) &&
    printerBuildVolume === null;
  const clearSplitWorkflow = useSplitFaceStore((state) => state.clearForModelReplacement);
  const clearManualDraftForModelReplacement = useManualDraftStore(
    (state) => state.clearForModelReplacement,
  );
  const resetAutomaticDraft = useAutomaticDraftStore((state) => state.resetStrategy);
  const clearSplitWorkflowForOrientation = useSplitFaceStore(
    (state) => state.clearForOrientationChange,
  );

  // Below: whichever draft is active during an open More Molds tab
  // (Automatic's own viewport rendering stays deferred -- see
  // CuttingSessionPanel's doc comment -- so only Manual redirects here).
  // Both the singleton and Manual's own hook are always called (Rules of
  // Hooks), the active one is picked via plain ternary.
  const singletonBaseReferenceMoldDefinition = useSplitFaceStore((state) => state.definition);
  const manualBaseReferenceMoldDefinition = useManualDraftStore((state) => state.definition);
  const baseReferenceMoldDefinition = isManualDraftActive
    ? manualBaseReferenceMoldDefinition
    : singletonBaseReferenceMoldDefinition;
  const singletonActiveMoldBodies = useSplitFaceStore(selectActiveMoldBodies);
  const manualActiveMoldBodies = useManualDraftStore(selectManualActiveMoldBodies);
  const activeMoldBodies = isManualDraftActive ? manualActiveMoldBodies : singletonActiveMoldBodies;
  const singletonSprueCavityGeometry = useSplitFaceStore((state) =>
    state.cavity.status === "complete"
      ? state.cavity.result?.cavityTool ?? null
      : null,
  );
  const manualSprueCavityGeometry = useManualDraftStore((state) =>
    state.cavity.status === "complete"
      ? state.cavity.result?.cavityTool ?? null
      : null,
  );
  const sprueCavityGeometry = isManualDraftActive
    ? manualSprueCavityGeometry
    : singletonSprueCavityGeometry;
  const singletonSelectionBoxAllowedByCavity = useSplitFaceStore(
    (state) =>
      state.cavity.status !== "generating" &&
      state.cavity.status !== "complete",
  );
  const manualSelectionBoxAllowedByCavity = useManualDraftStore(
    (state) =>
      state.cavity.status !== "generating" &&
      state.cavity.status !== "complete",
  );
  const selectionBoxAllowedByCavity = isManualDraftActive
    ? manualSelectionBoxAllowedByCavity
    : singletonSelectionBoxAllowedByCavity;
  const selectionBoxVisible =
    selectionBoxAllowedByCavity;
  const singletonRegistrationState = useSplitFaceStore((state) => state.registration);
  const manualRegistrationState = useManualDraftStore((state) => state.registration);
  const registrationState = isManualDraftActive ? manualRegistrationState : singletonRegistrationState;
  const singletonSpruePresentation = useSplitFaceStore(selectSpruePresentationDefinitions);
  const manualSpruePresentation = useManualDraftStore(selectManualSpruePresentationDefinitions);
  const spruePresentation = isManualDraftActive ? manualSpruePresentation : singletonSpruePresentation;
  ...
  const singletonSplitWorkflow = useSplitFaceStore((state) => state.workflow);
  const manualSplitWorkflow = useManualDraftStore((state) => state.workflow);
  const splitWorkflow = isManualDraftActive ? manualSplitWorkflow : singletonSplitWorkflow;
  const singletonSelectedSplitFaces = useSplitFaceStore((state) => state.selectedFaceIds);
  const manualSelectedSplitFaces = useManualDraftStore((state) => state.selectedFaceIds);
  const selectedSplitFaces = isManualDraftActive ? manualSelectedSplitFaces : singletonSelectedSplitFaces;
  const singletonCuttingPlanes = useSplitFaceStore((state) => state.cuttingPlanes);
  const manualCuttingPlanes = useManualDraftStore((state) => state.cuttingPlanes);
  const cuttingPlanes = isManualDraftActive ? manualCuttingPlanes : singletonCuttingPlanes;
  ...
  const manualClearanceMm = useManualDraftStore((state) => state.clearanceMm);
  const clearanceMm = isManualDraftActive ? manualClearanceMm : singletonClearanceMm;
  ...
  // Read-only: this viewport never calls requestMode/acceptPlan/executeAcceptedPlan.
  ...
  const resetSegmentationForMoldScale = useSegmentationModeStore(
    (state) => state.resetStrategy,
  );
  // Also requires the One Mold tab to actually be the active tab -- the
  // segmentation store's own plan/phase persist across tab switches within
  // an open session (each tab initializes once, on first visit, and keeps
  // its state while the user works in another tab -- see
  // cuttingWorkflow.store.ts's ensureOneMoldTabInitialized), so without this
  // its preview/committed bodies would keep rendering after switching away,
  // violating "inactive tab drafts must not appear in the viewport."
  const segmentationVisualizationActive = useIsSegmentationVisualizationActive() && isOneMoldTabActive;
  ...
  const manualToggleSplitFace = useManualDraftStore((state) => state.toggleFace);
  const toggleSplitFace = isManualDraftActive
    ? manualToggleSplitFace
    : singletonInteractionBlocked
      ? blockedVoidInteraction
      : singletonToggleSplitFace;
  const singletonBeginPlaneDrag = useSplitFaceStore((state) => state.beginPlaneDrag);
  const manualBeginPlaneDrag = useManualDraftStore((state) => state.beginPlaneDrag);
  const beginPlaneDrag = isManualDraftActive
    ? manualBeginPlaneDrag
    : singletonInteractionBlocked
      ? blockedVoidInteraction
      : singletonBeginPlaneDrag;
  const singletonCommitPlaneDrag = useSplitFaceStore((state) => state.commitPlaneDrag);
  const manualCommitPlaneDrag = useManualDraftStore((state) => state.commitPlaneDrag);
  const commitPlaneDrag = isManualDraftActive
    ? manualCommitPlaneDrag
    : singletonInteractionBlocked
      ? blockedVoidInteraction
      : singletonCommitPlaneDrag;
  const singletonCancelPlaneDrag = useSplitFaceStore((state) => state.cancelPlaneDrag);
  const manualCancelPlaneDrag = useManualDraftStore((state) => state.cancelPlaneDrag);
  const cancelPlaneDrag = isManualDraftActive
    ? manualCancelPlaneDrag
    : singletonInteractionBlocked
      ? blockedVoidInteraction
  ...
  const handleMoldScaleUpdate = useCallback(
    (clearanceMm: number) => {
      ...
      // plan/result immediately, but resetStrategy only cancels/clears state;
      // it does not dispatch geometry work.
      resetSegmentationForMoldScale();
      resetAutomaticDraft();
    },
    [resetAutomaticDraft, resetSegmentationForMoldScale, updateSingletonClearanceEdit],
  );
  const handleMoldScaleCommit = useCallback(() => {
    ...
    // drive requestMode/acceptPlan/executeAcceptedPlan). One Mold/Automatic
  ...
  const handleModelStatusChange = useCallback(
    (nextStatus: ModelImportStatus) => {
      ...
      if (
        ...
        resetViewportTool();
        clearSplitWorkflow();
        // Model replacement invalidates every draft, not just whichever is
        // active -- a stale Automatic/Manual draft must never survive a
        // swapped model.
        clearManualDraftForModelReplacement();
        resetAutomaticDraft();
      }
  ...
  const handlePlaneCommit = useCallback((id: string, normalized: number) => {
    ...
    // while a segmentation strategy owns the model) -- forward its new
  ...
  }, [cancelPlaneDrag, commitPlaneDrag, cuttingPlaneK1, isManualDraftActive]);
  ...
  const {
    ...
    // Only the Cut by Face tab (the singleton, directly) or More Molds'
    // Manual sub-strategy (its own draft) may ever expose interactive
    // face-selection -- One Mold and More Molds' Automatic sub-strategy
    // have no face-based cutting planes of their own, and outside any open
    // session there is nothing to select (Cut by Face interaction now lives
    // entirely inside the panel's own tab, not on the idle main toolbar).
    splitFaceSelectionActive: cuttingGuidesVisible &&
      (isCutByFaceTabActive || isManualDraftActive) &&
    ...
    // while a segmentation tab (One Mold, or More Molds' Automatic
    // sub-strategy) owns the model -- the two cutting systems must not
    // appear active at the same time (see segmentationPreviewPlanes/
  ...
  // A committed One Mold (or More Molds Automatic) segmentation result is
  // now promoted into the singleton imperatively, inside
  // cuttingWorkflow.store.ts's commitActiveTab, at the moment Done commits
  // it -- not reactively here. Doing it there instead of in a `useEffect`
  // watching segmentationPhase/segmentationResult removes an entire class
  // of self-invalidation race: the promotion's own write to the singleton
  // used to retrigger segmentationMode.store.ts's own handleUpstreamChange
  // staleness subscription, which could mark the just-executed result stale
  // before the reactive effect's promotion had even run. commitActiveTab's
  // single synchronous sequence (execute, then promote) has no such gap.
