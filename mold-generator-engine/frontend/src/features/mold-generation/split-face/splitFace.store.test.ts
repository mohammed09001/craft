...
describe("face-driven mold workflow",()=>{
 ...
 it("processes hidden bodies and invalidates cavity after an upstream plane move",async()=>{const canonicalPartGeometry=canonicalCube("m",k1);const s=useSplitFaceStore.getState();s.setCanonicalPartGeometrySignature(canonicalPartGeometry.sourceSignature);s.enterSelection();s.toggleFace("right");await useSplitFaceStore.getState().createMoldParts("m",k1);const hidden=useSplitFaceStore.getState().definition!.moldBodies![0]!.id;useSplitFaceStore.getState().setBodyVisibility(hidden,false);await useSplitFaceStore.getState().createCavity(canonicalPartGeometry);expect(selectActiveMoldBodies(useSplitFaceStore.getState())?.find(b=>b.id===hidden)?.visible).toBe(false);const plane=useSplitFaceStore.getState().cuttingPlanes[0]!;useSplitFaceStore.getState().beginPlaneDrag(plane.id);useSplitFaceStore.getState().commitPlaneDrag(plane.id,.5,k1);expect(useSplitFaceStore.getState().cavity.status).toBe("unavailable");expect(useSplitFaceStore.getState().definition).toBeNull();expect(selectActiveMoldBodies(useSplitFaceStore.getState())?.find(b=>b.id===hidden)?.visible).toBe(false);});
...
describe("adoptCommittedSegmentationResult (oversized/segmented cavity bridge)",()=>{
 ...
 it("preserves Sprue intent (sprueDefinitions) across a committed segmentation adoption instead of wiping it to [], regression for lost Sprue intent on One Mold/Automatic re-commit",async()=>{
  const canonicalPartGeometry=canonicalCube("m",k1);
  const s=useSplitFaceStore.getState();
  ...
  // A second committed segmentation plan (e.g. re-running One Mold after
 ...
 it("propagates the adaptive Segmentation Registration sizing policy into committed One Mold / Automatic More Molds evaluation, regression for the lost-policy defect",async()=>{
  const canonicalPartGeometry=canonicalCube("m",k1);
  const s=useSplitFaceStore.getState();
 ...
 it("rebuilds a truthful unsegmented base after Mold Scale invalidates a promoted segmentation result, instead of nulling the definition",async()=>{
  ...
  // Mold Scale: instant commit (setClearanceMm), matching the reported One
  // Mold / Automatic More Molds regression -- clearance changes must never
