...
export interface SplitFaceState extends Snapshot {
  ...
  seedCuttingPlanesForReopen(cuttingPlanes:readonly CuttingPlaneRecord[],clearanceMm:number):void;
 ...
 * definition (One Mold / Automatic More Molds) under a new clearance,
 ...
 * reruns One Mold or Automatic More Molds to obtain a new segmented result
 * from this base.
 */
...
function rebuildReferenceMoldForClearance(
 ...
 try{
  ...
  // keeps taking this branch too, not just the first one. Cut by Face and
  // Manual More Molds never set segmentationLineage, so this branch never
  // affects those paths.
 ...
 * above already uses to distinguish Segmentation-promoted definitions (One
 * Mold and Automatic More Molds, both sized the same way) from Cut by Face
 * and Manual More Molds (which never set it and must stay on the NORMAL
...
return (set,get)=>({...initial,
 ...
 enterSelection:()=>set(s=>({...history(s),workflow:editableWorkflow(s.cuttingPlanes.length),error:null,activePlaneId:null})), toggleFace:(id)=>set(s=>{
  ...
  return {
   ...
   error:null,
   activePlaneId:null,
   document:createDocument(s.document.revision+1,null,cuttingPlanes,s.clearanceMm,s.cavity.clearanceMm,s.sprueDefinitions),
   evaluation:s.evaluation.phase==="evaluating"?{...s.evaluation,phase:"stale"}:s.evaluation,
  };
 }),
 addExtensionCuttingPlane:(faceId,algorithmUsedAxes,suggestedNormalizedPosition)=>{
  ...
  // deliberately: One Mold and More Molds Automatic never call
 ...
 }, clearSelection:()=>{cancelDerivedMoldEvaluation("Mold document changed.");set(s=>s.cuttingPlanes.length?{...history(s),selectedFaceIds:[],selectedSplitFaceId:null,cuttingPlanes:[],workflow:"selectingFaces",definition:null,cavity:unavailableCavity(s.cavity.clearanceMm),registration:unavailableRegistration(),document:createDocument(s.document.revision+1,null,[],s.clearanceMm,s.cavity.clearanceMm,s.sprueDefinitions),evaluation:s.evaluation.phase==="evaluating"?{...s.evaluation,phase:"stale"}:s.evaluation,error:null,activePlaneId:null}:s);},
  ...
  */
 seedCuttingPlanesForReopen:(cuttingPlanes,clearanceMm)=>{
  cancelActiveCavityGeneration("Reopening a committed more-molds result.");
  cancelDerivedMoldEvaluation("Reopening a committed more-molds result.");
  const selectedFaceIds=PART_BOUNDING_BOX_FACE_IDS.filter(faceId=>cuttingPlanes.some(plane=>plane.sourceFaceId===faceId));
  set({...initial,cuttingPlanes,selectedFaceIds,selectedSplitFaceId:selectedFaceIds.at(-1)??null,clearanceMm,workflow:editableWorkflow(cuttingPlanes.length)});
 },
 /**
  * Promotes an already-validated, already-executed segmentation result
  * (an Automatic more-molds draft's "Done" commit, or an oversized-model
  * General Segmentation execution) into the singleton as its new
 ...
 * in-progress stage. Factory so a second live SplitFaceState-shaped store
 * (a "make it as more molds" Manual draft) gets its OWN independent
 * single-slot memoization instead of thrashing a shared module-level cache
 * against a different instance's state on every render -- confirmed unsafe
 * for two concurrently-subscribed live instances during the Phase 6
...
export function createSelectActiveMoldBodies() {
 ...
 return (state:Pick<SplitFaceState,"definition"|"lastCommittedResult"|"bodyVisibility">)=>{
  const source=state.lastCommittedResult?.bodies??state.definition?.moldBodies;
