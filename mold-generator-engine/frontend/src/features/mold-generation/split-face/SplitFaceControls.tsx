 ...
 * the panel) and are routed to whichever store the currently active
 * session tab/draft actually owns -- the exact same singleton/manual/
 * automatic selection CuttingSessionPanel already used when it hosted these
 * controls itself, just relocated, not reimplemented. Flip, Sprue, Create
 * Cavity, and the Constructed Cutting Plan trigger remain singleton-only,
 * post-commit tools -- disabled (not hidden) while a session is open, since
 * none of them apply to a draft that hasn't been committed yet.
 */
export function SplitFaceControls({
  ...
  const activeSessionTab = useCuttingWorkflowStore((s) =>
    s.state.kind === "sessionOpen" ? s.state.activeTab : null,
  );
  ...
  useEffect(() => {
    ...
    // owns a draft (Manual) or a planning lifecycle (Cut by Face/Segmentation),
    // so never allow the global command to remain active across that boundary.
    if (isSessionOpen && activeTool === "mold-scale") setActiveTool("pointer");
  }, [activeTool, isSessionOpen, setActiveTool]);

  // Undo/Redo route to whichever draft the More Molds tab currently owns
  // (Manual's own draft, or Automatic's) -- Cut by Face and One Mold both
  // drive the singleton directly (see CuttingSessionPanel's own doc
  // comment), so the singleton's own history already covers them, exactly
  // as it does while idle.
  const isManualDraftActive = useIsManualDraftActive();
  const manualUndo = useManualDraftStore((s) => s.undo);
  const manualRedo = useManualDraftStore((s) => s.redo);
  const manualCanUndo = useManualDraftStore((s) => s.undoStack.length > 0);
  const manualCanRedo = useManualDraftStore((s) => s.redoStack.length > 0);
  const automaticUndo = useAutomaticDraftStore((s) => s.undo);
  const automaticRedo = useAutomaticDraftStore((s) => s.redo);
  const automaticCanUndo = useAutomaticDraftStore((s) => s.undoStack.length > 0);
  const automaticCanRedo = useAutomaticDraftStore((s) => s.redoStack.length > 0);
  const moreMoldsTabActive = isSessionOpen && activeSessionTab === "moreMolds";
  const undo = moreMoldsTabActive ? (isManualDraftActive ? manualUndo : automaticUndo) : state.undo;
  const redo = moreMoldsTabActive ? (isManualDraftActive ? manualRedo : automaticRedo) : state.redo;
  const canUndo = moreMoldsTabActive
    ? (isManualDraftActive ? manualCanUndo : automaticCanUndo)
    : state.undoStack.length > 0;
  const canRedo = moreMoldsTabActive
    ? (isManualDraftActive ? manualCanRedo : automaticCanRedo)
