...
describe("segmentation execution lifecycle integration", () => {
  ...
  it("executes the accepted live-equivalent Y/Z One Mold plan through the store lifecycle", async () => {
    const sourceBounds = {
      min: { x: 0, y: 0, z: 0 },
      max: { x: 8, y: 24, z: 24 },
    } as const;
  ...
  it("executes multiple Y planes against the real committed source snapshot", async () => {
    ...
    const planned = useSegmentationModeStore.getState().requestMode("make-as-more-molds");
    expect(planned.status).toBe("planned");
    if (planned.status !== "planned") return;
