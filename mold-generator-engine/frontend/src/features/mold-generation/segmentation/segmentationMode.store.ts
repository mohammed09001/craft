...
/** Compatibility surface for the already-reviewed toolbar work. */
export type SegmentationStrategy = "one-mold" | "more-molds";

const MODE_BY_STRATEGY: Readonly<
  Record<SegmentationStrategy, SegmentationMode>
> = {
  "one-mold": "make-as-one-mold",
  "more-molds": "make-as-more-molds",
};

const STRATEGY_BY_MODE: Readonly<
  Record<SegmentationMode, SegmentationStrategy>
> = {
  "make-as-one-mold": "one-mold",
  "make-as-more-molds": "more-molds",
};
...
interface SegmentationModeSnapshot {
  readonly strategy: SegmentationStrategy | null;
  readonly mode: SegmentationMode | null;
...
export type SegmentationModeStore = SegmentationModeSnapshot & {
  ...
  requestMode: (mode: SegmentationMode) => SegmentationResult;
  setStrategy: (strategy: SegmentationStrategy) => void;
  acceptPlan: () => boolean;
 ...
 * extraction. Undo/redo covers the planning/execution lifecycle state (mode/phase/plan/result/
 * preview/registration) -- the same fields the singleton already treats as "the current
 * segmentation draft." Automatic does not yet have its own independent clearance parameter (it
 * still reads the shared splitFace singleton's clearanceMm for mold-frame geometry, same as
 * "make it as one mold" does today) -- that is a deliberately deferred follow-up, not silently
 * assumed, since wiring it would require making `segmentationSourceSnapshot.ts`'s clearance read
 * itself injectable, which is out of this phase's scope (proving draft isolation/commit/discard/
 * provenance against one engine, not building Automatic's own clearance UX).
 */
export function createSegmentationModeStoreCreator(
  ...
  return (set, get) => {
     ...
     * requestMode's own preview-generation path (epoch/phase-guarded so a
    ...
    requestMode: (mode) => {
      ...
      set({
        strategy: STRATEGY_BY_MODE[mode],
        mode,
        phase: "planning",
        request: null,
        plan: null,
      ...
      const result = requestSegmentationPlan(mode);
      const source = readCurrentSegmentationSourceSnapshot();
    ...
    setStrategy: (strategy) => {
      get().requestMode(MODE_BY_STRATEGY[strategy]);
    },
    acceptPlan: () => {
      const before = get();
    ...
    resetStrategy: () => {
      ...
      // -- resetStrategy is "start over," never a state a later undo should
...
function handleUpstreamChange() {
  ...
  if (wasActive && !isActive) {
    ...
    // retained -- reset fully, not just the strategy/mode selection, so no
    // stale plan/result/phase survives into the normal small-model workflow.
    store.resetStrategy();
  }
  wasActive = isActive;
}
