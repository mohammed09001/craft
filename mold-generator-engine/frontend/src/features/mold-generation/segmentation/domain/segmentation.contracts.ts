...
export interface CandidateScore {
   ...
   * SegmentationObjectiveWeights) -- 0 until a strategy's adjustCandidate
   ...
   * a deterministic fallback when strategies don't differentiate further.
   */
  readonly weightedObjectiveScore: number;
}

/**
 * Strategy-owned, adjustable objective weights consumed by a
 * SegmentationModeStrategy's own adjustCandidate to compute
 * CandidateScore.weightedObjectiveScore -- the generic engine never
 * hard-codes these; each strategy supplies (and may vary) its own. All
 * weights are non-negative; a larger weight makes that term matter more in
 * the final (lower-is-better) score.
 */
export interface SegmentationObjectiveWeights {
  ...
  /**
   * Rewards candidates whose segment count is closer to the algorithm's own
   * estimated-useful section count for logical (non-printer-fit-forced)
   * axes -- "logical section quality" / "independent mold usefulness" for
   * More Molds, meaningless (weight 0) for One Mold.
   */
  readonly sectionQualityWeight: number;
}
...
export interface ModeExtensionOutput {
  readonly mode: SegmentationMode;
  readonly status: "deferred";
  readonly obligations: readonly string[];
}

export interface SegmentationModeStrategy {
  readonly mode: SegmentationMode;
  readonly policyId: string;
  readonly policyVersion: number;
  /**
   * When true, this strategy wants the algorithm to identify logical
   * sections (SegmentationAlgorithm.identifyLogicalSections) and plan for
   * them even when the mold already fits the printer on its own -- i.e.
   * printer-fit is not the only reason to segment. False for One Mold
   * (segmentation is only ever printer-fit-driven there); true for More
   * Molds Automatic.
   */
  readonly considerSectionDrivenSegmentation: boolean;
  /**
   * Strategy-owned scoring weights consumed by this strategy's own
   * adjustCandidate to compute CandidateScore.weightedObjectiveScore. The
   * generic engine (planSegmentation.ts) never reads this directly -- it
   * only ever reads the resulting score field.
   */
  readonly objectiveWeights: SegmentationObjectiveWeights;
  adjustCandidate(
  ...
  createExtensionOutput(plan: SegmentationPlan): ModeExtensionOutput;
}
...
export interface SegmentationAlgorithm {
  ...
  /**
   * Deterministic, conservative "logical sectioning" heuristic: given the
   * mold's own size and the axes printer-fit already forces, suggest which
   * additional axes are worth sectioning on for reasons independent of
   * printer size (e.g. pronounced elongation), and how many pieces each
   * suggested axis should be split into. Keys are axes NOT already in
   * printerFitAxes; the suggested count (how many logical pieces make
   * sense) is the algorithm's own call, not the generic planner's --
   * planSegmentation.ts only ever takes max(printerFitCount, this count)
   * per axis, never a hard-coded default of its own. Optional -- an
   * algorithm that doesn't implement section-driven planning simply omits
   * it, and callers must treat a missing method the same as one returning
   * an empty object.
   */
  identifyLogicalSections?(
    sourceSize: Size3,
    printerFitAxes: readonly FitAxis[],
  ): Readonly<Partial<Record<FitAxis, number>>>;
}
...
export type SegmentationPlanningResult =
  | {
      ...
      readonly extension: ModeExtensionOutput;
    }
  | {
