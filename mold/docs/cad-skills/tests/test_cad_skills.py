...
PRODUCTION_CONSTRAINT_PROFILE_SCHEMA_PATH = (
    CONTRACTS_DIR / "production-constraint-profile.schema.json"
)
SEGMENTATION_POLICY_SCHEMA_PATH = CONTRACTS_DIR / "segmentation-policy.schema.json"
SEGMENTATION_REQUEST_SCHEMA_PATH = (
    CONTRACTS_DIR / "general-segmentation-request.schema.json"
)
SEGMENTATION_PLAN_SCHEMA_PATH = CONTRACTS_DIR / "general-segmentation-plan.schema.json"
LOOP_ARTIFACT_ENVELOPE_SCHEMA_PATH = (
    CONTRACTS_DIR / "loop-artifact-envelope.schema.json"
)
...
EXECUTION_ENGINE_REGISTRY_SCHEMA_PATH = (
    REGISTRY_DIR / "execution-engine-registry.schema.json"
)
...
SEGMENTATION_SKILL_PATH = (
    REPOSITORY_ROOT / ".claude" / "skills" / "general-segmentation-engine" / "SKILL.md"
)
...
class JSONSchemaValidator:
    ...
    def validate(instance: dict, schema: dict, path: str = "root") -> list[str]:
        ...
        if isinstance(schema_type, list):
            ...
            if not matched:
                return [
                    f"{path}: value {instance} does not match allowed union types {schema_type}"
                ]
            return []
...
def validate_segmentation_plan_semantics(plan: dict, profile: dict) -> list[str]:
    ...
    candidates = {
        candidate["candidateId"]: candidate for candidate in plan["candidates"]
    }
    ...
    if decision == "NOT_REQUIRED" and any(
        candidate["boundaryIntents"] for candidate in plan["candidates"]
    ):
...
def validate_segmentation_traceability(artifact: dict, artifact_kind: str) -> list[str]:
    ...
    if artifact_kind == "implementation":
        trace_plan_id = artifact["implementationRecord"][
            "implementationTraceability"
        ].get("segmentationPlanArtifactId")
        if not metadata_plan_id or metadata_plan_id != trace_plan_id:
            return [
                "segmentation implementation must carry one consistent accepted plan reference"
            ]
...
def run_phase_2_tests():
    ...
    all_schemas = [
        ...
        (
            "Production Constraint Profile Schema",
            PRODUCTION_CONSTRAINT_PROFILE_SCHEMA_PATH,
        ),
    ...
    for skill_id, (expected_caps, expected_status) in canonical_skills.items():
        ...
        assert actual_caps == expected_caps, (
            f"Skill {skill_id} capabilities mismatch: expected {expected_caps}, got {actual_caps}"
        )
        ...
        print(
            f"  PASS: Skill '{skill_id}' confirmed {expected_status} with capabilities {actual_caps}."
        )

    # 2b. Cross-registry drift guard: skill-registry.json and .claude/loop/registry.json
    # independently maintain the same 9 CAD-pipeline skills' routing metadata.
    # `description` legitimately differs between them (each is written for a
    # different consumer's framing), but `capabilities`, `dependencies`, and
    # `executionPriority` are mechanical facts about the same skill and must not
    # silently diverge. This does not merge the two registries — see
    # docs/cad-skills/LOOP_ENGINE.md for why a full merge is out of scope — it
    # only turns "must be kept in sync manually" into a failure an agent cannot miss.
    loop_registry_for_drift_check = load_json(LOOP_REGISTRY_FILE_PATH)
    loop_map_for_drift_check = {
        s["skillId"]: s for s in loop_registry_for_drift_check["skills"]
    }
    ...
    print(
        f"  PASS: capabilities/dependencies/executionPriority agree across both registries for all {len(canonical_skills)} CAD-pipeline skills."
    )
    ...
    sample_s3 = {
        ...
        "implementationRecord": {
            ...
            "modifiedFiles": [
                "frontend/src/features/mold-generation/sprue-generation/sprueProfile.ts"
            ],
            "createdFiles": [
                "frontend/src/features/mold-generation/sprue-generation/sprueGeneration.contracts.ts"
            ],
            "exports": ["createSprueProfile", "validateSprueBounds"],
            "implementationTraceability": {
                "algorithmArtifactId": "ALG-SPRUE-001",
                "geometryArtifactId": "GEO-SPRUE-001",
                "specSectionsSatisfied": [
                    "1_problemDefinition",
                    "2_inputs",
                    "6_algorithmPipeline",
                ],
    ...
    print(
        "  PASS: Skill 3 ImplementationRecord satisfies schema rules and preserves traceability."
    )
    ...
    sample_s4_fail["validationReport"]["testResults"][2]["errorDetails"] = (
        "Euler characteristic violation: V - E + F = 0 (Non-manifold edge detected)"
    )
    sample_s4_fail["validationReport"]["geometryValidation"][
        "watertightManifoldCheck"
    ] = False
    ...
    print(
        "  PASS: Skill 4 correctly detects and formats intentional topology validation failures."
    )
    ...
    sample_s5 = {
        ...
        "optimizationReport": {
            ...
            "appliedOptimizations": [
                {
                    "technique": "SPATIAL_QUERY",
                    "description": "BVH spatial ray query acceleration",
                    "speedupFactor": 2.33,
                }
            ],
    ...
    assert (
        sample_s5["optimizationReport"]["behaviorVerification"][
            "observableBehaviorUnchanged"
        ]
        is True
    )
    assert (
        sample_s5["optimizationReport"]["behaviorVerification"]["contractsUnchanged"]
        is True
    )
    print(
        "  PASS: Skill 5 OptimizationReport satisfies schema rules and proves observable behavior remains unchanged."
    )
    ...
    print(
        "  PASS: Skill 6 MaintenanceHealthReport satisfies schema rules and detects technical debt."
    )

    # 7. End-to-End Traceability Chain Assertion Across Skills 1-6
    print("\n--- TEST 7: End-to-End Artifact Traceability Chain Assertion ---")
    traceability_chain = {
        "Skill 1 (Design)": sample_s3["implementationRecord"][
            "implementationTraceability"
        ]["algorithmArtifactId"],
        "Skill 2 (Geometry)": sample_s3["implementationRecord"][
            "implementationTraceability"
        ]["geometryArtifactId"],
        "Skill 3 (Implementation)": sample_s4_pass["metadata"][
            "sourceImplementationArtifactId"
        ],
    ...
    print(
        "  PASS: Complete traceability chain verified across Skills 1 -> 2 -> 3 -> 4 -> 5 -> 6!"
    )
    ...
    print(
        "  PASS: Prompt Compiler payload structurally valid and conforms to PromptCompilerContract schema."
    )
    ...
    sample_proposal = {
        ...
        "validationPlan": [
            "npm test -- --run src/features/viewport",
            "npm run typecheck",
        ],
        "risks": [
            "Registration key material may be shared with an unrelated feature role"
        ],
        "openQuestions": [
            "Is the color shared across all key instances or assigned per-body?"
        ],
        "executionOrder": [
            {
                "step": 1,
    ...
    assert sample_proposal["version"] == 2, (
        "REVISE must increment version, never reuse it"
    )
    assert len(sample_proposal["history"]) == 1, (
        "Prior version must be preserved in immutable history, not discarded"
    )
    assert sample_proposal["history"][0]["version"] == 1
    assert sample_proposal["history"][0]["status"] == "REJECTED"
    print(
        "  PASS: EngineeringProposal satisfies schema rules; REVISE preserves immutable version history."
    )
    ...
    stale_case["approval"]["approvedPromptHash"] = (
        "c" * 64
    )  # does not match promptHash below
    stale_case["approval"]["approvedVersion"] = stale_case["version"]
    stale_case["approval"]["approvedRepositoryRevision"] = stale_case[
        "repositoryRevision"
    ]

    errs_stale = JSONSchemaValidator.validate(stale_case, proposal_schema)
    assert len(errs_stale) == 0, (
        f"Stale-approval fixture Validation Errors: {errs_stale}"
    )
    ...
    print(
        "  PASS: Proposal contract carries the fields needed to detect a stale approval (promptHash mismatch)."
    )
    ...
    for required_engine in ("claude-code", "codex", "gemini-cli"):
        assert required_engine in engines_by_id, (
            f"Missing execution engine entry: {required_engine}"
        )
    ...
    print(
        "  PASS: Execution Engine Registry structurally valid; capability claims match repository evidence."
    )
    ...
    executing_proposal["approval"]["approvedPromptHash"] = executing_proposal[
        "promptHash"
    ]
    executing_proposal["approval"]["approvedVersion"] = executing_proposal["version"]
    executing_proposal["approval"]["approvedRepositoryRevision"] = executing_proposal[
        "repositoryRevision"
    ]
    ...
    assert len(errs_executing) == 0, (
        f"Executing Proposal Validation Errors: {errs_executing}"
    )
    assert (
        executing_proposal["approval"]["approvedVersion"]
        == executing_proposal["version"]
    ), (
    ...
    assert (
        "EditFiles"
        in engines_by_id[executing_proposal["executionEngineId"]]["capabilities"]
    ), (
        "The assigned execution engine must actually declare the capability required to execute a code change"
    )
    print(
        "  PASS: EXECUTING proposal with executionEngineId satisfies schema rules and references a registered, capable engine."
    )
    ...
    assert len(errs_validating) == 0, (
        f"VALIDATING Proposal Validation Errors: {errs_validating}"
    )
    print(
        "  PASS: VALIDATING proposal is schema-valid (real code path in .claude/loop/loop-trace.mjs's begin-validation)."
    )
    ...
    assert len(errs_completed) == 0, (
        f"COMPLETED Proposal Validation Errors: {errs_completed}"
    )
    print(
        "  PASS: COMPLETED proposal is schema-valid (real code path in .claude/loop/loop-trace.mjs's complete-validation)."
    )
    ...
    assert (
        set(proposal_schema["properties"]["status"]["enum"]) == all_lifecycle_states
    ), (
    ...
    print(
        "  PASS: All 10 declared lifecycle states are accounted for (see .claude/loop/loop-trace.test.mjs for the Node-side transition tests proving each is a real, reachable code path)."
    )
    ...
    print(
        "  PASS: JSONSchemaValidator now correctly rejects a type mismatch on an integer|null union field (bug fixed this pass)."
    )
    ...
    for profile in (cnc_profile, robotics_profile):
        ...
        assert profile_errors == [], (
            f"Production profile validation errors: {profile_errors}"
        )
    print(
        "  PASS: materially different CNC and robotics profiles use the same domain-neutral contract."
    )
    ...
    segmentation_policy = {
        ...
        "tieBreakers": [
            ...
            "minimum-hard-constraint-margin",
            "engineering-risk",
            "candidate-id",
        ],
    ...
    assert policy_errors == [], (
        f"Segmentation policy validation errors: {policy_errors}"
    )
    missing_policy = json.loads(json.dumps(segmentation_policy))
    del missing_policy["components"]
    assert JSONSchemaValidator.validate(missing_policy, policy_schema), (
        "Missing mandatory policy components must fail"
    )
    ...
    print(
        "  PASS: policy priorities are explicit; missing policy and unowned extension data are rejected."
    )
    ...
    segmentation_request = {
        ...
        "downstreamRequirements": [
            "preserve subject identity in the execution manifest"
        ],
    ...
    assert request_errors == [], (
        f"Segmentation request validation errors: {request_errors}"
    )
    ...
    required_plan = {
        ...
        "postExecutionValidationRequirements": [
            "coverage conservation",
            "downstream manifest integrity",
        ],
        "warnings": [],
        "blockers": [],
    ...
    print(
        "  PASS: necessity, selected-candidate, blocker, segment-limit, and no-execution semantics are enforced."
    )
    ...
    for strategy_family in (
        ...
        strategy_request["candidateStrategySources"][0]["strategyFamily"] = (
            strategy_family
        )
    ...
    print(
        "  PASS: multiple externally owned strategy families use the same request and plan contracts."
    )
    ...
    print(
        "  PASS: project extension data is namespaced and owner/versioned; partial updates require prior-plan lineage at runtime."
    )

    segmentation_implementation = json.loads(json.dumps(sample_s3))
    segmentation_implementation["featureName"] = "General Segmentation"
    segmentation_implementation["metadata"]["sourceSegmentationPlanArtifactId"] = (
        "SEG-PLAN-001"
    )
    ...
    assert (
        validate_segmentation_traceability(
            segmentation_implementation, "implementation"
        )
        == []
    )
    ...
    assert validate_segmentation_traceability(
        missing_implementation_plan, "implementation"
    )

    segmentation_validation = json.loads(json.dumps(sample_s4_pass))
    segmentation_validation["featureName"] = "General Segmentation"
    segmentation_validation["metadata"]["sourceSegmentationPlanArtifactId"] = (
        "SEG-PLAN-001"
    )
    ...
    assert (
        validate_segmentation_traceability(segmentation_validation, "validation") == []
    )
    ...
    print(
        "  PASS: segmentation implementation and validation carry the same accepted plan traceability."
    )
    ...
    assert (
        segmentation_loop_entry["inputContract"]
        == segmentation_registry_entry["inputContract"]
    )
    assert (
        segmentation_loop_entry["outputContract"]
        == segmentation_registry_entry["outputContract"]
    )
    assert (
        segmentation_loop_entry["dependencies"]
        == segmentation_registry_entry["dependencies"]
    )
    assert segmentation_loop_entry["executionPriority"] == 25
    assert (
        loop_map["interconnected-systems-stability-engineer"]["executionPriority"]
        < loop_map["cad-algorithm-implementer"]["executionPriority"]
    )
    assert (
        "interconnected-systems-stability-engineer"
        not in segmentation_loop_entry["dependencies"]
    )
    ...
    assert (
        "Never define, invent, or hard-code a candidate-generation method" in skill_text
    )
    ...
    print(
        "  PASS: registries agree; planning precedes project stability and implementation without a Mold-specific dependency."
    )

    print("\n=======================================================================")
    print(
        "  ALL TESTS PASSED SUCCESSFULLY! SKILLS PIPELINE & GATEWAY INTEGRATED (PASS) "
    )
    print("=======================================================================")
