...
describe("authoritative mold document integration", () => {
  ...
  it("8 clears Sprue intent when a topology edit removes the committed definition it was authored against", async () => { await parts(); await useSplitFaceStore.getState().createSprue(pendingPlacement()); expect(useSplitFaceStore.getState().sprueDefinitions).toHaveLength(1); useSplitFaceStore.getState().enterSelection(); useSplitFaceStore.getState().toggleFace("right"); expect(useSplitFaceStore.getState().sprueDefinitions).toHaveLength(0); expect(useSplitFaceStore.getState().sprues).toHaveLength(0); expect(useSplitFaceStore.getState().lastCommittedResult).toBeNull(); expect(selectActiveMoldBodies(useSplitFaceStore.getState())).toBeUndefined(); });
  it("9 retains Sprue intent but invalidates stale derived bodies when mold clearance changes", async () => { await parts(); await useSplitFaceStore.getState().createSprue(pendingPlacement()); useSplitFaceStore.getState().setClearanceMm(12); expect(useSplitFaceStore.getState().sprueDefinitions).toHaveLength(1); expect(useSplitFaceStore.getState().lastCommittedResult).toBeNull(); const rebuilt = selectActiveMoldBodies(useSplitFaceStore.getState()); expect(rebuilt).toEqual(useSplitFaceStore.getState().definition?.moldBodies); });
  ...
  it("16 repeated evaluation starts from unchanged cavity geometry", async () => { await cavity(); const versions=useSplitFaceStore.getState().cavity.result!.bodies.map(body=>body.geometryVersion); await useSplitFaceStore.getState().createSprue(validPlacement()); await vi.waitFor(()=>expect(useSplitFaceStore.getState().sprues).toHaveLength(1)); const id=useSplitFaceStore.getState().sprueDefinitions[0]!.operationId; await useSplitFaceStore.getState().resizeSprue(id,3); await vi.waitFor(()=>expect(useSplitFaceStore.getState().sprues[0]!.profile.mainDiameterMm).toBe(3)); expect(useSplitFaceStore.getState().cavity.result!.bodies.map(body=>body.geometryVersion)).toEqual(versions); });
