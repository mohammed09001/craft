...
/**
 * Execution 05 Article 04: scale-aware geometry tolerance derivation. Pure
 * numeric-tolerance math over a bounding box and a clearance value -- the
 * contract makes sense without any product-domain vocabulary. Domain layers
 * (Cavity, Master Mold) may wrap this with their own versioned policy labels.
 */

export const GEOMETRY_TOLERANCE_POLICY_VERSION = "geometry-adaptive-v2";

export interface GeometryTolerancePolicy {
   ...
   * against the analytical (Float64) reference block. Manifold stores mesh
...
/** Builds conservative, scale-aware millimeter tolerances for one Boolean geometry operation. */
export function buildGeometryTolerancePolicy(
  bounds: Bounds3,
  clearanceMm: number,
  ...
  if (
    ...
    throw new Error("Geometry tolerance policy inputs are invalid.");
  }
  ...
  // A block can sit far from the origin even when its own dimensions/diagonal
  // are small (translated-but-modest-size mold), so the Float32 precision floor
  ...
  // is exactly the defect being fixed, regardless of how large the block is.
  ...
  const policy: GeometryTolerancePolicy = {
  ...
  if (Object.values(policy).some(value => !Number.isFinite(value) || value <= 0)) {
    throw new Error("Geometry tolerance policy could not be resolved safely.");
  }
