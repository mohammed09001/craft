...
import {
  SEGMENTATION_EXTENSION_OBLIGATIONS,
} from "./segmentation.contracts";
import { deterministicSegmentationId } from "./segmentationIdentity";
import {
  SEGMENTATION_OBJECTIVE_WEIGHTS,
  computeWeightedObjectiveScore,
} from "./segmentationObjectiveScoring";
...
export function planSegmentation(input: {
  ...
  const { request, source, algorithm } = input;
  ...
  // Segmentation is printer-fit-driven only: an axis is segmented exactly
  // when the mold does not fit the printer along it.
  const requiredAxes: readonly FitAxis[] =
  ...
  const plan: SegmentationPlan = {
    ...
    extensionObligations: SEGMENTATION_EXTENSION_OBLIGATIONS,
  };
