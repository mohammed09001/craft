...
describe("segmentation planning", () => {
  ...
  it("changes identity when the source revision changes", () => {
    const baseline = plan();
    ...
    if (baseline.status !== "planned" || revised.status !== "planned") {
      return;
    }
    expect(revised.plan.id).not.toBe(baseline.plan.id);
  });
  ...
  it("rejects a baseline candidate that crosses a hard protected region", () => {
    ...
    const result = plan(source({ x: 250, y: 50, z: 50 }, [protectedRegion]));
  ...
  it("changes request identity when protected-region evidence changes", () => {
    ...
    const withRegion = plan(source({ x: 250, y: 50, z: 50 }, [region]));
  ...
  it("orchestrates an injected algorithm without hard-coding a concrete implementation", () => {
    const injected: SegmentationAlgorithm = {
      ...baselineSegmentationAlgorithm,
      id: "test-replaceable-algorithm",
    };
    const result = plan(source(), injected);
    expect(result.status).toBe("planned");
    if (result.status !== "planned") return;
  ...
  it("carries a planned multi-boundary X sequence into execution preflight", () => {
    ...
    const result = plan(sourceSnapshot);
    expect(result.status).toBe("planned");
    if (result.status !== "planned") return;
...
describe("segmentation is printer-fit-driven only", () => {
  // Fits every printer axis (100x100x100) on its own, even though x is
  // significantly more elongated than y/z -- a fitting mold is never
  // segmented, no matter its proportions.
  const elongatedButFittingSource = source({ x: 95, y: 50, z: 50 });

  it("does not plan a fitting, elongated mold", () => {
    const result = plan(elongatedButFittingSource);
...
describe("real objective-policy candidate ranking", () => {
  ...
  it("applies the planning objective weights when ranking otherwise-equal candidates", () => {
    const injected: SegmentationAlgorithm = {
      ...baselineSegmentationAlgorithm,
    ...
    expect(result.status).toBe("planned");
    if (result.status !== "planned") return;
    // The planning weights reward a larger margin (marginWeight > 0), so
    // the high-margin candidate wins -- proving the weights (not a
    // hard-coded engine rule) drove the selection.
