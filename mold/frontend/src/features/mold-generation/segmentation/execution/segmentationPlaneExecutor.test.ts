...
describe("segmentation plane executor", () => {
  ...
  it("executes the live-equivalent Y/Z Cartesian Segmentation grid with the planned nine bodies", async () => {
    const sourceBounds = {
      min: { x: 0, y: 0, z: 0 },
      max: { x: 8, y: 24, z: 24 },
    } as const;
  ...
  it("runs Create Cavity Boolean generation after fractional translated Segmentation segmentation", async () => {
    const sourceBounds = {
      min: { x: 10, y: 20, z: 30 },
      max: { x: 18, y: 45, z: 55 },
    } as const;
  ...
  it.each([
    ...
    const printerVolume = { x: 8.4, y: 8.4, z: 8.4 };
    const { source, plan } = fixture(sourceBounds, printerVolume);
  ...
  it("uses the same Cartesian targeting contract for every plan", async () => {
    const sourceBounds = {
      min: { x: 0, y: 0, z: 0 },
      max: { x: 8, y: 24, z: 24 },
    } as const;
    const printerVolume = { x: 8.1, y: 8.1, z: 8.1 };
    const { source, plan } = fixture(sourceBounds,
      printerVolume,
    );
  ...
  it("supports three Y planes and defers fit only for a future Y target", async () => {
    ...
    const { source, plan } = fixture(yBounds,
      printerVolume,
    );
    const request = executionRequest(
  ...
  it("supports three Z planes and defers fit only for a future Z target", async () => {
    ...
    const { source, plan } = fixture(zBounds,
      printerVolume,
    );
    const request = executionRequest(
  ...
  it("produces a mode-free executor request shape", () => {
    const one = fixture();
