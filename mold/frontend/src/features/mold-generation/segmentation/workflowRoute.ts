 ...
 * re-deriving this list against `segmentation.store.ts`'s own phases.
 */
...
export function useIsSegmentationVisualizationActive(): boolean {
  const plan = useSegmentationStore((state) => state.plan);
  const phase = useSegmentationStore((state) => state.phase);
